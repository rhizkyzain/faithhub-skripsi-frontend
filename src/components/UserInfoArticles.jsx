...
const UserInfo = ({ openId, index, setOpenId, article, answer }) => {
  ...
  return (
    <div className="w-full  flex items-cente justify-between">
      <div className="w-[48%] md:max-w-screen-md posted-by flex items-center gap-2 md:gap-3">
        ...
        <h2 className="text-gray-300 text-xs">
          ...
          <span className="text-blue-800 font-bold  md:text-sm">
            {article
              ? article.ownerInfo.name === currentUser?.data.name
                ? article?.ownerInfo?.name + " (You)"
                : article?.ownerInfo?.name
              : answer
      ...
      <div className="posted-on mx-auto">
        <h2 className="text-gray-300 text-xs">
          {article
            ? moment(article.articleDetails.createdAt).fromNow()
      ...
      {openId && (
        ...
        >
          ...
          <span className="text-gray-300 text-xs">
            {article?.articleDetails.replyCount || "No replies"}
          </span>
        </div>
      )}
    </div>
